...
# - Primer afegim les dependències a les altres taules
...
# - Ara afegim l'entrada que ens han demanat
...
-- Insertem dades a la taula user
...
-- Busquem quines entrades hi ha a transaction que no són a la taula user
SELECT transaction.user_id as Taula_transaction, user.id AS taula_user
FROM transaction
LEFT JOIN user
	ON transaction.user_id = user.id
WHERE user.id is NULL;

-- Afegim l'entrada que faltava
INSERT into user (id)
VALUES ('9999');

-- Enllacem les taules
alter table transaction add constraint fk_user_id foreign key (user_id)
